...
class OnvifController:
    ...
    async def _init_single_camera(self, cam_name: str) -> bool:
        ...
        try:
            ...
            self.cams[cam_name] = {
                ...
                "features": [],
                "presets": {},
                # HIKVISION FIX: Initialize tracking variables
                "last_pan": 0.0,
                "last_tilt": 0.0,
                "last_move_ts": 0.0,
            }
    ...
    async def get_camera_status(self, camera_name: str) -> None:
        async with self.status_locks[camera_name]:
            ...
            # HIKVISION FIX: POSITION BASED TRACKING LOGIC STARTS HERE
            pan_tilt_status = None
            try:
                # 1. Try getting the standard MoveStatus
            ...
            # 2. Overwrite using Coordinate Delta Logic
            try:
                current_pan = status.Position.PanTilt.x
                current_tilt = status.Position.PanTilt.y
                
                last_pan = self.cams[camera_name].get("last_pan", 0)
                last_tilt = self.cams[camera_name].get("last_tilt", 0)
                
                # Check absolute difference (epsilon 0.001 is small movement)
                delta = abs(current_pan - last_pan) + abs(current_tilt - last_tilt)
                
                now = time.time()
                
                if delta > 0.001:
                    pan_tilt_status = "MOVING"
                    self.cams[camera_name]["last_move_ts"] = now
                else:
                    # If we haven't moved in this check, are we within the buffer?
                    # 100ms buffer (0.1s)
                    if now - self.cams[camera_name].get("last_move_ts", 0) < 0.15:
                        pan_tilt_status = "MOVING"
                    else:
                        pan_tilt_status = "IDLE"
                
                # Update state
                self.cams[camera_name]["last_pan"] = current_pan
                self.cams[camera_name]["last_tilt"] = current_tilt
                
            except Exception as e:
                # Fallback to original logic if we can't read Position
                logger.debug(f"Hikvision Fix: Could not read Position: {e}")
                pass

            # Original Logic continues, but now pan_tilt_status is forced to correct value
            zoom_status = getattr(status.MoveStatus, "Zoom", None)

            try:
                 # Check if we have a valid status now
                if pan_tilt_status is None or pan_tilt_status not in [
                    "IDLE",
                    "MOVING",
                ]:
                    # This used to raise Exception, but we suppress it if we fixed it via coordinates
                    logger.warning(f"Invalid PTZ Status: {pan_tilt_status}")
                    raise Exception
            except Exception:
                logger.warning(
